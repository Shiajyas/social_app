...
export default function Spam() {
  ...
  const dismissMutation = useMutation({
    ...
    onSuccess: (_, rId) => {
      qc.setQueryData<Report[]>(["reports", page], prev => prev?.filter(r => r._id !== rId) || []);
      decRep(); toast.success("✅ Report dismissed");
    },
  ...
  const blockMutation = useMutation({
    ...
    onSuccess: (_, pId) => {
      ...
      decRep(); toast.success("🚫 Post blocked");
    },
  ...
  useEffect(() => {
    ...
    socket.on("admin:newReport", (report: Report) => {
      ...
      incRep(); toast.info("📨 New report received");
    });
    return () => socket.off("admin:newReport");
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {selected.size > 0 && (
        <div className="flex gap-4 mb-6">
          <Button variant="destructive" onClick={dismissSelected} disabled={dismissMutation.isPending}>
                  ...
                  >
                    <CardContent className="p-5 space-y-3">
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-300">
                        Owner: <strong className="text-gray-800 dark:text-gray-100">{report.post.owner.username}</strong>
                      </div>
                    </CardContent>
                  </Card>
                );
      ...
      <div className="flex justify-between items-center mt-6">
        <Button onClick={() => setPage(p => Math.max(p - 1, 1))} disabled={page === 1 || isFetching}>⬅ Previous</Button>
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => setPage(p => p + 1)} disabled={reports.length < PAGE_LIMIT || isFetching}>Next ➡</Button>
        </div>
      </div>
    </div>
  );
