...
import { GroupDocument as IGroups } from '../../../core/domain/interfaces/IGroups';
export class GroupSocketService implements IGroupSocketService {
  ...
  async createGroup(socket: Socket, data: IGroups, callback: Function): Promise<void> {
    try {
  ...
  async updateGroup(socket: Socket, data: IGroups, callback: Function): Promise<void> {
    try {
...
async addMember(
  ...
  try {
    ...
    socket.emit('group-member-added', {
     ...
     socket.broadcast.emit('group-member-added', { groupId, memberId });
  ...
  async removeMember(socket: Socket, groupId: string, memberId: string, callback: Function): Promise<void> {
    try {
      ...
      socket.emit('group-member-removed', { groupId, memberId });
       socket.broadcast.emit('group-member-removed', { groupId, memberId });
...
async getGroupMembers(socket: Socket, groupId: string): Promise<void> {
 ...
 async sendCommunityMessage(
    ...
    try {
      ...
      const populated = await this._MessageRepository.getMessageById( savedMessage._id);
      ...
      //  socket.to(groupId).emit('new-message', populated);
        socket.emit('new-message', populated); // ✅ send back to sender too
  ...
  async receiveMessage(
    ...
    try {
      const fullMessage = await this._MessageRepository.getMessageById(message._id as string);
